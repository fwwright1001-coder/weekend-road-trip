...
(() => {
  ...
  // Bot 3 owns the canonical "fuel low" flag (state.fuelLow). Consume it when
  // present; fall back to a local threshold so the warning still works before
  // that integration lands. Read-only — we never write Bot 3's flag.
  const LOW_FUEL_WARN_FRAC = 0.25;
  function isFuelLow() {
    if (typeof state.fuelLow === 'boolean') return state.fuelLow;
    return state.fuel > 0 && state.fuel <= FUEL_MAX * LOW_FUEL_WARN_FRAC;
  }
  ...
  function updateGame(dt) {
    ...
    // Low-fuel warning (edge-triggered): beep + announce once on crossing into
    // low, re-armed after a refuel. Consumes Bot 3's read-only fuel-low flag.
    const lowNow = isFuelLow();
    if (lowNow && !state._lowFuelWarned) {
      state._lowFuelWarned = true;
      audio.playLowFuel();
      announce('Low fuel');
    } else if (!lowNow) {
      state._lowFuelWarned = false;
  ...
  // ============================================================
  // Enable DEBUG via "?debug" in the URL or localStorage 'wrt.debug' = '1' to
  // auto-run on boot. window.runSelfTests() / window.WRT.runSelfTests() also run
  // it manually. Tests are NON-DESTRUCTIVE: anything written to real storage is
  // snapshotted and restored, and no AudioContext is created without a user
  // gesture. Returns { pass, fail, total, results } and logs a grouped summary.
  const DEBUG = (() => {
    try {
